...
- **Public Surface**: `Sources/Tessera` exposes the SwiftUI-facing API. `Tessera` defines the tile configuration (size, items, spacing, density, seed). `TesseraItem` wraps individual SwiftUI content with weighting, rotation and scaling rules. `TesseraPattern` is the primary view; it repeats a tile across a `Canvas`.
- **Rendering Pipeline**: `TesseraPattern` constructs a single cached symbol (`TesseraCanvasTile`) and draws it in a grid that covers the available space. The tile itself runs deterministic randomness via `SeededGenerator` so the same seed reproduces the same layout.
...
- **Determinism and Reuse**: Seeds propagate from `Tessera` into `TesseraPattern` and internal generators, ensuring repeatable outputs while keeping symbol resolution cached for performance during canvas redraws.
