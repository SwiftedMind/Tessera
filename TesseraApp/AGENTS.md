...
- Use `monocle inspect --file <path> --line <line> --column <column> --json` to resolve a symbol.
- Line and column values are 1-based and the column must point inside the identifier.
- Use `monocle` especially for symbols involved in errors/warnings or coming from dependencies (including the Tessera package).
