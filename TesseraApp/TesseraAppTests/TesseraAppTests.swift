...
struct TesseraAppTests {
  ...
  @Test func uploadedImageRoundTripPersistsEmbeddedAsset() async throws {
    let embeddedAssetIdentifier = UUID()
    let embeddedAssetFileExtension = "jpg"
    let embeddedAssetData = Data([9, 8, 7, 6, 5])
    ...
    let itemPayload = EditableItemPayload(
      id: UUID(),
      customName: "Image File",
      presetID: "uploadedImage",
      isVisible: true,
      weight: 1,
      minimumRotation: 0,
      maximumRotation: 360,
